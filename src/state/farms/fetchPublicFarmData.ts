...
type PublicFarmData = {
  ...
  depositFee: string
  withdrawFee: string
  mtwPerBlock: SerializedBigNumber
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const [info, totalAllocPoint, mtwPerBlock] =
    pid || pid === 0
  ...
  const MtwPerBlock = mtwPerBlock ? new BigNumber(mtwPerBlock).div(BIG_TEN.pow(18)) : BIG_ZERO
  ...
  return {
    ...
    mtwPerBlock: MtwPerBlock.toJSON(),
  }
}
